...
function Categories({ swal }) {
  ...
  function editCategory(category) {
    ...
    setProperties(
      category.properties.map(({ name, values }) => ({
        id: uuidv4(), // Generate a unique ID for each property
        name,
        values: values.join(","),
      }))
    );
  ...
  const handlePropertyNameChange = (propertyId, newName) => {
    setProperties((prev) => {
  ...
  const handlePropertyValuesChange = (propertyId, newValues) => {
    setProperties((prev) => {
      return prev.map((property) => {
        if (property.id === propertyId) {
  ...
  function removeProperty(propertyId) {
    setProperties((prev) => {
      return prev.filter((property) => property.id !== propertyId);
    });
  }
  return (
    <Layout>
